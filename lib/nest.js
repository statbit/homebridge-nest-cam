...
const BetaServerInfo =  {
  server: 'webapi.camera.home.ft.nest.com',
  sessionKey: 'website_ft'
}

const ProductionServerInfo = {
  server: 'webapi.camera.home.nest.com',
  sessionKey: 'website_2'
}


class NestAPI extends EventEmitter {
  constructor(username, password, useBeta) {
    super();
    let self = this;
    ...
    if(useBeta) {
      self.serverInfo = BetaServerInfo;
    } else {
      self.serverInfo = ProductionServerInfo;
    }
  ...
  sendHomeRequest(endpoint, method, body) {
    let self = this;
    return self.sendRequest(self.serverInfo.server, endpoint, method, body);
  }
  ...
  sendRequest(hostname, endpoint, method, body) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (self.sessionToken !== undefined) {
        headers['Cookie'] = self.serverInfo.sessionKey + '=' + self.sessionToken;
      }
